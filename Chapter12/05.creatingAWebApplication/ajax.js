...
function formatted(sum) {
    if (sum.toString().indexOf(".") < 0) { sum += ".00"; }
    if (sum.toString().indexOf(".") === (sum.toString().length - 2)) { sum += "0"; }
    return sum;
}
...
function totalize() {
    ...
    for (i = 0; i < nums.length; i++) { sum += parseFloat(nums[i].firstChild.data); }
...
function update() {
    ...
    if (row === 0) { panel.innerHTML = "Select a row"; return; }
    if (col === 0) { panel.innerHTML = "Select a column"; return; }
    if (!new_value) { panel.innerHTML = "Enter a value"; return; }
    if (isNaN(new_value)) { panel.innerHTML = "Enter number"; return; }
